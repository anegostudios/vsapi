...
namespace Vintagestory.API.Client
{
    public abstract class GuiElementEditableTextBase : GuiElementTextBase
    {
        ...
        {
            (int selectionStart, int selectionEnd) = GetSelection(caretPos);
            SetValue(GetText().Remove(selectionStart, selectionEnd -  selectionStart), false);
            selectedTextStart = null;
            if (caretPos == selectionEnd) CaretPosWithoutLineBreaks = selectionStart + caretPosOffset;
        }
        ...
        {
            return (Math.Min(this.selectedTextStart.Value, caretPos), Math.Max(this.selectedTextStart.Value, caretPos));
        }
        ...
        public override void OnMouseDownOnElement(ICoreClientAPI api, MouseEvent args)
        {
            ...
            bool shiftDown = new Func<bool>(() => {
                try
                {
                    return api.Input.KeyboardKeyStateRaw[(int)GlKeys.ShiftLeft] || api.Input.KeyboardKeyStateRaw[(int)GlKeys.ShiftRight];
                }
                catch (NotImplementedException ex)
                {
                    // TODO: this crashes in main menu, it would be good to get the raw state implemented
                    return api.Input.KeyboardKeyState[(int)GlKeys.ShiftLeft] || api.Input.KeyboardKeyState[(int)GlKeys.ShiftRight];
                }
            })();
        ...
        private void OnKeyDownInternal(ICoreClientAPI api, KeyEvent args)
        {
            ...
            if (args.KeyCode == (int)GlKeys.BackSpace || args.KeyCode == (int)GlKeys.Delete)
            {
            ...
            if (args.ShiftPressed != selectedTextStart.HasValue && (args.KeyCode == (int)GlKeys.Up || args.KeyCode == (int)GlKeys.Down || args.KeyCode == (int)GlKeys.Left || args.KeyCode == (int)GlKeys.Right || args.KeyCode == (int)GlKeys.Home || args.KeyCode == (int)GlKeys.End))
            {
        ...
        private bool OnControlAction(KeyEvent args)
        {
            ...
            if (keyString == "c" || keyString == "x") return OnCopyCut(keyString == "c" ? CopyCutMode.Copy : CopyCutMode.Cut);
        ...
        private bool OnCopyCut(CopyCutMode mode) {
            ...
            int caretPos = CaretPosWithoutLineBreaks;
            (int selectionStart, int selectionEnd) = GetSelection(caretPos);
            string subtext = text[selectionStart..selectionEnd];
        ...
        protected void RenderTextSelection()
        {
            ...
            void RenderSelectionLine(int fromX, int toX, int lineIndex) {
                ...
                api.Render.Render2DTexturePremultipliedAlpha(selectionTextureId, x, y, width, height);
            }
            ...
            int caretPos = CaretPosWithoutLineBreaks;
            (int startX, int startY) = GetPosition(Math.Min(selectedTextStart.Value, caretPos));
            (int endX, int endY) = GetPosition(Math.Max(selectedTextStart.Value, caretPos));
            if (startY == endY)
            {
